...
```{r checkprimary_old,message=FALSE,warning=FALSE,attr.source='.numberLines'}
...
Write these data to the e-commerce database using **append=TRUE** so that the data will not overwrite the data type we set when creating empty tables.
...
Before writing data into the database, we would check the uniqueness of primary key for all of the files.
...
The objective here is to utilize a GitHub repository to manage our project. We connected our file on Posit Cloud with the GitHub work space, and used 'push' and 'pull' to control the version and synchrinize necessary files and script to run in the workflow.
...
7.  Rank product by their sold quantity
...
8.  Rank categories by sold quantity
...
9.  Rank parent categories by sold quantity
...
The below customers showcased a substantial level of engagement with the platform, consistently placing a significant number of orders. These insights offers valuable opportunities for targeted marketing efforts and customer retention strategies. Recognizing and incentivizing these active customers can foster loyalty ,contribute to enhanced customer satisfaction and long-term business success.
...
5.  Warehouse Capacity and Current Stock
...
ggplot(Product, aes(x = product_price)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black", alpha = 0.7) +  # Adjust fill color and transparency
  geom_vline(xintercept = mean_price, linetype = "dotted", color = "darkred") +  # Add mean line
...
product_ratings <- product_ratings[product_ratings$avg_rating >= 3,]
