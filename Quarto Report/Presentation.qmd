...
Read .csv files from the data_upload folder and write them into the database by using **append = TRUE** function so that these data will not destroy the data type we set before; make sure that columns such as ids are character instead of number, so that when writing them to the database, they will not show in decimal format.
...
The workflow action would look like the image below, updating whenever we push an updated script, edit the workflow code, update new data to the data_upload folder, or for every 12 hours.
...
![](Add%20plot%20figure.png)

![](Add%20plot%20figure2.png)

# Data Analysis

## Advanced Data Analysis with SQL and R
...
Analyzing sales performance by parent category helps businesses understand broader category popularity and sales volume trends. Businesses can design proper marketing campaign or promotion to boost sales for underperformed categories or further strengthen popular categories' sales.
...
This diagram further summarizes the total units of products sold for each parent category. Analyzing sales performance at the parent category level provides businesses with information into overall product category performance and the market trend.
...
```{sql connection=my_connection}
-- calculate the referred number of each customer and order from highest to lowest
SELECT 
...
This bar plot represents the comparison between warehouse capacity (in blue) and current stock quantities (in pink) for each warehouse; we can easily compare the stock level with the diagram. Understanding the capacity versus actual stock levels is vital for inventory management and logistics planning. It helps businesses ensure optimal stock levels, and avoid stock-outs or overstocking situations.
...
This plot provides information into the spread of product prices within categories and highlights the mean price with a dotted red line for easy comparison. This information can be helpful for the company to target the correct market segment (low-price, middle-price, or high-price/luxury market) and can be a reference for potential suppliers when setting product prices.

```{r}
# calculate the mean price
mean_price <- mean(Product$product_price)

# plot the price distribution and mark the mean with a vertical dotted line
...
This visualization is valuable to businesses as it enables targeted resource allocation and marketing strategies based on the concentration of customers in different cities, enhancing market penetration; it also helps in identifying expansion opportunities.
...
```{r}
# Ensure that data type are the same before joinning tables together
...
# use ggolot to visualize the calculated result
